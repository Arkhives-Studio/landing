...
export function Hero() {
  ...
  const apiBaseUrl = useMemo(() => {
    const fallback = import.meta.env.PROD
      ? 'https://email-service-aqls.onrender.com'
      : 'http://localhost:4000';
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/emails`, {
        ...
        body: JSON.stringify({
          email,
          source: audienceConfig[audience].source,
        }),
      });
        ...
        <motion.div
          ...
          <Tabs
            ...
            <TabsList className='grid w-full grid-cols-3 mb-8 bg-card/80 backdrop-blur-sm'>
              <TabsTrigger value='gamers' className='text-sm md:text-base'>
                Gamers
              </TabsTrigger>
              <TabsTrigger value='investors' className='text-sm md:text-base'>
                Investors
              </TabsTrigger>
              <TabsTrigger value='studios' className='text-sm md:text-base'>
                Studios
              </TabsTrigger>
            </TabsList>

            {(
              Object.entries(audienceConfig) as [
                AudienceKey,
                (typeof audienceConfig)[AudienceKey],
              ][]
            ).map(([key, config]) => (
